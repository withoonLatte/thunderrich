    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            />
            <div className="absolute -bottom-1 -right-1 bg-world-cup-gold text-blue-900 text-[10px] font-black px-1.5 py-0.5 rounded-md shadow-lg">
              #{user.role === 'admin' ? 'ADM' : 'PRO'}
            </div>
          </div>
          <div>
            <h2 className="text-lg font-black text-white italic tracking-tight">{user.displayName}</h2>
            <div className="flex items-center gap-1.5">
              <Trophy className="w-3.5 h-3.5 text-world-cup-gold" />
              <span className="text-world-cup-gold font-black text-xl italic">{user.points} <small className="text-[10px] uppercase font-bold opacity-70">คะแนน</small></span>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-end">
          ...
          <span className="text-xs font-black text-white tracking-widest">{wrongCount}/24</span>
        </div>
        ...
        {isBanned && (
          <div className="mt-4 bg-red-600/20 border border-red-600/30 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] leading-tight text-red-200 font-black uppercase tracking-wider mb-0.5">ถูกแบนจากการทำนาย</p>
              <p className="text-xs text-white font-medium">ถูกแบนเป็นจำนวน {user.bannedMatchIds.length} แมตช์</p>
            </div>
          </div>
        )}
      </div>
